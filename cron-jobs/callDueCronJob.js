...
const VoiceResponse = require("twilio/lib/twiml/VoiceResponse.js");
...
const callDueCronJob = cron.schedule("0 * * * *", async () => {
  try {
    // Find all tasks that are not soft-deleted and overdue
    const overdueTasks = await TaskModified.find({
      deleted_at: null,
